...
let app = http.createServer((req,res)=>{
    ...
    if(pathname === '/' || /(\.\w+)$/.test(pathname)){
        ...
        // ajax接口
        let database = __dirname+'/json/customer.json' ;
       ...
       let con = fs.readFileSync(__dirname+ '/json/customer.json','utf8'
            );
        ...
        if(pathname === '/api/removeInfo'){
            ...
            // splice删除 splice(index,个数)
            // 利用findIndex找到指定id的这一项的索引,findIndex 返回满足条件这一项的索引,如果没有找到返回-1 
        
           let delIndex = con.findIndex(item => +item.id === +id ); 
            // let delIndex = con.findIndex(item => +item.id === +id);
            console.log(delIndex);
            con.splice(delIndex,1);
            // 把删除完之后的结果下进数据库中
        ...
        if(pathname === '/api/addInfo'){
            ...
            req.on('end',()=>{
                ...
                if(isReg){
                    ...
                    con.push(data);
                    fs.writeFileSync(database,JSON.stringify(con));
                    res.end(JSON.stringify(d4));
                }
            });
        }
        ...
        if(pathname === '/api/modifyInfo'){
            ...
            req.on('data',chunk => dataStr += chunk);
            req.on('end',()=>{
                data = JSON.parse(dataStr); // 解析post的数据
                
            
            console.log(data);
            let { id } = data;
            let con = JSON.parse(fs.readFileSync(database,'utf8'));
            // 修改: 把客户端指定id的那一条数据直接替换成客户端传过来的数据
            let itemIndex = con.findIndex(item => +item.id === +id);
            // 找到修改的那一条数据额索引
            // console.log(data);
            con[itemIndex] = data;
            // 替换成客户端传过来的数据
